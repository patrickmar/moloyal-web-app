...
const SidebarNav = () => {
  ...
  return (
    <Sidebar className="bg-white fixed top-0 left-0 z-20 flex flex-col flex-shrink-0  w-64 h-full font-normal duration-75 lg:flex transition-width">
                ...
                {menu.submenus != undefined && menu.submenus.length > 0 ? (
                  <Sidebar.Collapse icon={menu.icon} label={menu.name}>
                    {menu.submenus.map((sm, id) => (
                      <Sidebar.Item key={id} as={Link} href={sm.link}>
                        {sm.name}
                      </Sidebar.Item>
                    ))}
                  </Sidebar.Collapse>
                ) : (
                  <Sidebar.Item icon={menu.icon} as={Link} href={menu.link}>
                    {" "}
                    {menu.name}{" "}
                  </Sidebar.Item>
                )}
